import { useSelector } from "react-redux";
import { selectSidebar } from "../../ControlSidebar/model/sidebarSlice";

function AddMediaIcon() {
  return (
    <svg
      data-encore-id="icon"
      role="img"
      aria-hidden="true"
      fill="#b3b3b3"
      viewBox="0 0 16 16"
      width="16"
      height="16"
    >
      <path d="M15.25 8a.75.75 0 0 1-.75.75H8.75v5.75a.75.75 0 0 1-1.5 0V8.75H1.5a.75.75 0 0 1 0-1.5h5.75V1.5a.75.75 0 0 1 1.5 0v5.75h5.75a.75.75 0 0 1 .75.75"></path>
    </svg>
  );
}

export function CreateMedia() {
  const { isSidebarOpen } = useSelector(selectSidebar);

  return (
    <div>
      <button
        title="Create playlist"
        className={`${isSidebarOpen ? "create-media-button-open" : "create-media-button-closed"} opacity-75 hover:transition hover:duration-75 hover:ease-in-out hover:opacity-100`}
      >
        <AddMediaIcon />
        <span
          className={isSidebarOpen ? "font-bold text-white text-sm" : "hidden"}
        >
          Create
        </span>
      </button>
    </div>
  );
}
