...
import { setShuffleMedia } from "../../model/mediaControlSlice";
import { selectMediaShuffled } from "../../model/mediaControlSlice";

function ShuffleMediaIcon() {
  const isMediaShuffled = useSelector(selectMediaShuffled);
  return (
    <svg
      data-encore-id="icon"
      role="img"
      ...
      width="24"
      height="24"
      fill={isMediaShuffled ? "#1ed760" : "#b3b3b3"}
      className={`scale-130 ${isMediaShuffled ? "shuffleIconActive" : "shuffleIcon"}`}
    >
...
export function ShuffleMedia() {
  const dispatch = useDispatch();
  return (
    <button
      onClick={() => dispatch(setShuffleMedia())}
      className="text-center ml-5 size-8 rounded-full cursor-poitner transition duration-75 hover:scale-104"
    >
